...
void cache_sim_t::init()  // initializing the cache
{
  ...
  size_t i = 0;
  access_order.resize(sets);
  for (auto& order : access_order)
  {
      order.resize(ways);
      i = 0;
...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  uint64_t* set_tags = &tags[idx * ways];
...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store)
{
  ...
  size_t set_index = (addr >> idx_shift) & (sets - 1);
...
void cache_sim_t::update_order(size_t set_index, size_t way_index) {
    auto begin_it = access_order[set_index].begin();
    auto end_it = access_order[set_index].end();
    auto it = std::find(begin_it, end_it, way_index);

    if (it != end_it) {
        access_order[set_index].erase(it);
    }
    access_order[set_index].insert(begin_it, way_index);
}
